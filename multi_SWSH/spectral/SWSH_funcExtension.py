...
def int_funcExtension(s, sw_fun):
    ...
    return np.vstack((sw_fun, \
            (-1)**s*np.flipud(np.roll(sw_fun[1:-1,:], np.int(N_ph/2),1))))        
...
def int_funcDoubleExtension(s, sw_fun):
    ...
    #Now extend again:
    sw_fun_T2_E = np.repeat(sw_fun_T2, 2, axis=1)
    return np.repeat(sw_fun_T2_E, 2, axis=0)
...
def h_int_funcExtension(s, sw_fun):
    ...
    sw_fun_E = np.hstack((sw_fun, -sw_fun))
    ...
    sw_fun_E = np.vstack((sw_fun_E, \
        s_ph \
        *np.roll(np.flipud(sw_fun_E[:-1,:]), np.int(N_ph/2), axis=1)))

    #Extend to full domain
    sw_fun_E = np.vstack((sw_fun_E, -sw_fun_E[1:-1,:]))
...
if __name__ == '__main__':
    ...
    ########
    s = 2
    int_fun = lambda th, ph: 3*np.exp(-1j*ph)*(3*np.sin(th)+2*np.sin(2*th) + \
        7*np.sin(3*th) - 7*np.sin(4*th))/(32*np.sqrt(2*np.pi))
    ...
    int_fun_N = np.zeros((N_th_E, N_ph), dtype=np.complex128)
    
    for i in np.arange(0, N_th_E):
        for j in np.arange(0, N_ph):
            int_fun_N[i,j] = int_fun(int_th[i], int_ph[j])

    #Throw small values
    int_fun_N[np.abs(int_fun_N)<1e-14] = 0
    ...
    diff_int_ext = np.max(np.abs(int_fun_N_ext - int_fun_N))
    ...
    #############
    h_s = -3/2
    h_int_fun = lambda th, ph: 1/40*np.exp(-3/2*1j*ph)*np.sqrt(3/np.pi)* \
        np.cos(th/2)*(-20*np.exp(2*1j*ph)*(-1+np.cos(th)) + 11*np.sqrt(2)* \
        np.cos(th/2)**2*(-3+5*np.cos(th)))
    ...
    #We sample the function on the extended domain directly
    h_int_fun_N = np.zeros((h_N_th_EE, h_N_ph_E), dtype=np.complex128)
    for i in np.arange(0, h_N_th_EE):
        for j in np.arange(0, h_N_ph_E):
    ...
    #Check difference in extension and full analytical specification on torus
    diff_h_int_ext = np.max(np.abs(h_int_fun_N_ext - h_int_fun_N))
